	...
	public class Person : GnossOCBase
	{
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			foreach(var item0 in this.Schema_networth)
			{
				...
				{
					foreach(var item2 in item0.Eschema_identifier)
					{
						AgregarTripleALista($"{resourceAPI.GraphsUrl}items/PriceSpecification_{ResourceID}_{item0.ArticleID}", "https://schema.org/extended/identifier", $"\"{GenerarTextoSinSaltoDeLinea(item2)}\"", list, " . ");
					}
				}
			}
			}
