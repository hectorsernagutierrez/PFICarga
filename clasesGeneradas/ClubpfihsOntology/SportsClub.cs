...
namespace ClubpfihsOntology
{
	...
	public class SportsClub : GnossOCBase
	{
		...
		public SportsClub(SemanticResourceModel pSemCmsModel, LanguageEnum idiomaUsuario) : base()
		{
			...
			this.Schema_foundingDate = new List<DateTime>();
			SemanticPropertyModel propSchema_foundingDate = pSemCmsModel.GetPropertyByPath("https://schema.org/foundingDate");
			if (propSchema_foundingDate != null && propSchema_foundingDate.PropertyValues.Count > 0)
			{
				foreach (SemanticPropertyModel.PropertyValue propValue in propSchema_foundingDate.PropertyValues)
				{
					DateTime fecha = new DateTime();
					DateTime.TryParse(propValue.Value,out fecha);
					this.Schema_foundingDate.Add(fecha);
				}
			}
		...
		public  List<DateTime> Schema_foundingDate { get; set;}
		...
		internal override void GetProperties()
		{
			...
			foreach (DateTime fecha in this.Schema_foundingDate){
				propList.Add(new DateOntologyProperty("schema:foundingDate", fecha));
			}
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			foreach(var item0 in this.Schema_parentOrganization)
			{
				...
				if(item0.IdsEschema_i_winner != null)
				{
					foreach(var item2 in item0.IdsEschema_i_winner)
					{
			...
			foreach(var item0 in this.Schema_location)
			{
				...
				{
					foreach(var item2 in this.Schema_foundingDate)
					{
						AgregarTripleALista($"{resourceAPI.GraphsUrl}items/SportsClub_{ResourceID}_{ArticleID}", "https://schema.org/foundingDate", $"\"{item2.ToString("yyyyMMddHHmmss")}\"", list, " . ");
					}
				}
		...
		public override List<string> ToSearchGraphTriples(ResourceApi resourceAPI)
		{
			...
			foreach(var item0 in this.Schema_location)
			{
				...
				if(this.Schema_foundingDate != null)
				{
					foreach(var item2 in this.Schema_foundingDate)
					{
						AgregarTripleALista($"http://gnoss/{ResourceID.ToString().ToUpper()}", "https://schema.org/foundingDate", $"{item2.ToString("yyyyMMddHHmmss")}", list, " . ");
					}
				}
				if(this.Schema_name != null)
				{
